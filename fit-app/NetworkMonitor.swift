...
class NetworkMonitor: ObservableObject {
    ...
    private func logNetworkChange(wasConnected: Bool, currentStatus: NWPath.Status) {
        ...
        if !wasConnected && isCurrentlyConnected {
            #if DEBUG
            Logger.network.debug("🌐 ✅ Red CONECTADA - Tipo: \(self.connectionType.emoji) \(self.connectionType.description)")
            ...
            #if DEBUG
            Logger.network.debug("🌐 📡 Cambio de red - Tipo: \(self.connectionType.emoji) \(self.connectionType.description)")
            #endif
        }
    }
